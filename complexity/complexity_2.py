...
from base_function.complexity.group2.main_180 import main_180
from base_function.complexity.group2.main_160 import main_160
from base_function.complexity.group2.main_140 import main_140
from base_function.complexity.group2.main_120 import main_120
from base_function.complexity.group2.main_100 import main_100
import value
...
model = initialize.Model(2,90,180,5,3,100,0.38,4)
...
EC_choose = [[3,5,2,5,3,2,1,1,4,5,3,1,2,1,3,2,3,5,1,1,4,3,4,2,2,4,5,2,3,3,2,4,5,2,1,4,1,1,5,2,1,4,1,1,4,5,3,1,5,2,1,4,4,1,1,2,4,5,1,2,3,5,1,1,4,3,3,2,2,4,5,2,3,2,3,3,2,4,1,1,4,5,3,2,3,3,2,4,4,1],[1,2,5,2,1,4,4,5,5,5,3,5,1,1,4,3,3,1,3,4,2,3,5,1,1,1,4,5,3,4,2,3,5,1,1,4,3,3,2,2,4,5,2,3,3,2,4,4,1,1,5,2,1,4,3,5,2,1,5,5,1,5,4,5,3,5,2,1,5,5,2,2,4,5,2,3,3,2,5,2,1,4,4,4,1,1,2,4,4,5]]
SES_utility,SES_choosed,group_SES,EC_x_res,iter_180_2 = main_180(model,EC_choose,group_utility,value.SES_x_all,value.SES_x_res,value.SES_cost,value.SES_price2,SES_utility)
y2_180_2 = [SES_utility[0],SES_utility[1],SES_utility[2],SES_utility[3],SES_utility[4]]


#初始化设置 共160人
model = initialize.Model(2,80,160,5,3,100,0.38,4)
EC_choose01 = np.zeros((model.group_num,model.num))
...
SES_utility,SES_choosed,group_SES,EC_x_res,iter_160_2 = main_160(model,EC_choose,group_utility,value.SES_x_all,value.SES_x_res,value.SES_cost,value.SES_price2,SES_utility)
y2_160_2 = [SES_utility[0],SES_utility[1],SES_utility[2],SES_utility[3],SES_utility[4]]


#初始化设置 共140人
model = initialize.Model(2,70,140,5,3,100,0.38,4)
EC_choose01 = np.zeros((model.group_num,model.num))
...
SES_utility,SES_choosed,group_SES,EC_x_res,iter_140_2 = main_140(model,EC_choose,group_utility,value.SES_x_all,value.SES_x_res,value.SES_cost,value.SES_price2,SES_utility)
y2_140 = [SES_utility[0],SES_utility[1],SES_utility[2],SES_utility[3],SES_utility[4]]


#初始化设置 共120人
model = initialize.Model(2,60,120,5,3,100,0.38,4)
EC_choose01 = np.zeros((model.group_num,model.num))
...
SES_utility,SES_choosed,group_SES,EC_x_res,iter_120_2 = main_120(model,EC_choose,group_utility,value.SES_x_all,value.SES_x_res,value.SES_cost,value.SES_price2,SES_utility)
y2_120 = [SES_utility[0],SES_utility[1],SES_utility[2],SES_utility[3],SES_utility[4]]


#初始化设置 共100人
model = initialize.Model(2,50,100,5,3,100,0.38,4)
EC_choose01 = np.zeros((model.group_num,model.num))
...
SES_utility,SES_choosed,group_SES,EC_x_res,iter_100_2 = main_100(model,EC_choose,group_utility,value.SES_x_all,value.SES_x_res,value.SES_cost,value.SES_price2,SES_utility)
y2_100_2 = [SES_utility[0],SES_utility[1],SES_utility[2],SES_utility[3],SES_utility[4]]


#图1中文
x = np.array([100, 120, 140, 160, 180])  

iter_2 = np.array([iter_100_2,iter_120_2,iter_140_2,iter_160_2,iter_180_2])
print("分为两组:", iter_2)
...
plt.legend(['$H=2$'],loc="best",fontsize=35) # 把标签加载到图中哪个位
...
plt.savefig("复杂度_2.png", dpi=500, bbox_inches='tight')    # 解决图片不清晰，不完整的问题
...
plt.grid(linestyle='-.')
plt.savefig("Comeplexity_2.png", dpi=500, bbox_inches='tight')    # 解决图片不清晰，不完整的问题
