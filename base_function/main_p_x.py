...
from SES_iterative_p_res import SES_iterative_p_res
from compute_x_res import compute_x_res


#主函数
def main_p_x(model,EC_choose,group_utility,SES_x_all,SES_x_res,SES_cost,SES_price,SES_utility):

    flag2 = 1
    iterate2 = 0
    SES_choosed_ = np.zeros(model.SES_num)
    SES_utility_lowest = compute_S_u_lowest(model,SES_x_all)  
    cur_dfx_p_ = np.zeros(model.SES_num)
    cur_dfx_x_ = np.zeros(model.SES_num)
    y_1_p = []
    x_1_x = []
    y_2_p = []
    x_2_x = []
    ...
    while flag2 == 1:
        ...
        while flag == 1:
            ...
            tran_flag = ( EC_transition <= 0.004 )  #值为 0 or 1
        ...
        if flag2 == 1:
            ...
            cur_dfx_p,cur_dfx_x,SES_price,SES_x_res = SES_iterative_p_res(model,EC_x_res,SES_choosed,SES_x_all,SES_x_res,SES_price,SES_cost,cur_dfx_p_,cur_dfx_x_)
            cur_dfx_p_ = cur_dfx_p
            cur_dfx_x_ = cur_dfx_x
    ...
    print("当前交易完成")
    print("\n")
    return SES_utility,SES_choosed,group_SES,EC_x_res,SES_price